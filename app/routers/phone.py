...
from app.services import phone_service, segment_service
...
async def get_phone(
    ...
    search = Phone(name="", number="", segment="")
    if q is not None:
        search.name = q
        search.number = q
        search.segment = q
        phones = phone_service.search(phone=search)
    else:
        phones = phone_service.get()
    return phones
...
@router.post(
    ...
    responses={
        status.HTTP_201_CREATED: {"model": Phone},
        status.HTTP_409_CONFLICT: {"model": Result},
    },
)
async def post_phone(
    item: Phone, user: Optional[UserInDB] = Depends(get_actual_user)
):
    ...
    ret = phone_service.create(item)
    item.id = ret.inserted_id
    ret = phone_service.getByID(item)
    return ret
...
async def delete_phone(
    id: PyObjectId, user: Optional[dict] = Depends(get_actual_user)
):
    item = Phone(name="", number="", segment="")
    item.username_update = user.username
    item.id = id
    if item.id is None:
        return get_error(
